...
abstract class _ActivityImageBean implements Bean<ActivityImage> {
  ...
  List<SetColumn> toSetColumns(ActivityImage model, {bool update = false, Set<String> only, bool onlyNonNull = false}) {
    List<SetColumn> ret = [];
    ...
    if (only == null && !onlyNonNull) {
      ...
      if (only.contains(activityId.name)) ret.add(activityId.set(model.activityId));
  ...
  Future<void> createTable({bool ifNotExists = false}) async {
    ...
    st.addInt(activityId.name, foreignTable: activityBean.tableName, foreignCol: 'id', isNullable: false);
    st.addInt(imageId.name, foreignTable: mediaFileBean.tableName, foreignCol: 'id', isNullable: false);
  ...
  Future<void> insertMany(List<ActivityImage> models, {bool onlyNonNull = false, Set<String> only}) async {
  ...
  Future<void> upsertMany(List<ActivityImage> models, {bool onlyNonNull = false, Set<String> only}) async {
    final List<List<SetColumn>> data = [];
  ...
  Future<void> updateMany(List<ActivityImage> models, {bool onlyNonNull = false, Set<String> only}) async {
    final List<List<SetColumn>> data = [];
  ...
  Future<List<ActivityImage>> findByActivity(int activityId, {bool preload = false, bool cascade = false}) async {
...
// Return if model has no pivots. If this is not done, all records will be removed!
  ...
  Future<List<ActivityImage>> findByMediaFile(int imageId, {bool preload = false, bool cascade = false}) async {
...
// Return if model has no pivots. If this is not done, all records will be removed!
  ...
  Future<dynamic> attach(MediaFile one, Activity two, {bool upsert = false}) async {
